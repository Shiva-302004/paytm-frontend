...
const SendAnotheruser = () => {
    ...
    const handleclick=()=>{
        fetch(`https://paytm-backend-br8r.onrender.com/api/auth/transaction/sendmoney/${params.id}`,{
            method:"POST",
            headers:{
            ...
            if(data.success){
                ...
                localStorage.setItem("balance",data.data1.amount)
  ...
  return (
    <div>
        ...
        <div className='flex flex-col justify-center  mt-[10vh] ml-2'>
            <h1 className='font-semibold text-xl'>Send Money to the {localStorage.getItem('username')}</h1>
        ...
        <Link className="bg-[#00b9f5] w-[93vw] md:w-[96vw] absolute bottom-0 h-[10vh] text-center text-white  ml-3" onClick={()=>setmodal(!modal)}>
            <div className='mt-3'>Send Money</div>
        </Link>
        <div className={`absolute   z-20 bg-yellow-50 p-2 h-[100vh] w-[100vw] top-0 text-center md:w-[60vw] md:h-[90vh] md:top-12 md:left-[20%] ${modal?"visible":"hidden"}`}>
            ...
            <div className='mt-3' >Confirm Payment</div>
        </Link>
        </div>
...
export default SendAnotheruser